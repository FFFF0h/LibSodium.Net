...
> **Note:** Libsodium provides two related but incompatible algorithms:
>
...
bool isValid = CryptoSign.Verify(message, signature, publicKey);
```
...
> `PreHashSign` and `PreHashVerify` use the Ed25519ph algorithm.
...
using var curveSk = new SecureMemory<byte>(CryptoBox.PrivateKeyLen);
...
- `LibSodiumException` — when signing, key generation, or key conversion cannot complete.
